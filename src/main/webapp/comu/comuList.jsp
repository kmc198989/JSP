		...
		<table class="list">
			...
			<c:forEach var="comu" items="${comulist}" varStatus="status">
				<tr class="record">
					...
					<td class="title-row"
						onclick="toggleContent('content_${comu.c_post_num}', this)">
						<a href="comuMain.do?code=${comu.c_post_num}"> <c:set
								var="truncatedTitle"
								value="${fn:substring(comu.c_title, 0, 20)}" />
							${truncatedTitle}${fn:length(comu.c_title) > 20 ? '...' : ''}
					</a>
					</td>
